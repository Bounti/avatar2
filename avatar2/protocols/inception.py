...
class InceptionProtocol(object):
    ...
    def write_memory(self, address, wordsize=4, value, num_words=1, raw=False):
        """
    ...
    def set_breakpoint(self, address, hardware=True):
...
class IPCortexM3(InceptionProtocol):
    ...
    def __init__(self, additional_args=[], 
        ...
        super(IPCortexM3, self).__init__()
    ...
    def reset(self):
        ...
        self.write_memory(0xE00ED0C, 4, ((0x5FA << 16) | (1 << 2)))
    ...
    def cont(self):
        ...
        # Set C_HALT bit at 1 in the DHCSR register.
    ...
    def stop(self):
        ...
        # Set C_HALT bit at 1 in DHCSR register
        self.write_memory(0xE000EDF0, 4, (0xA05F << 16) | (1 << 1) | (1 << 0))
        self._debug_enabled = True

        # Check the core acknowledges by reading S_HALT bit in DHCSR
        DHCSR = self.read_memory(0xE000EDF0, 4)
        if not (DHCSR & 0x20000):
            self.log.warning("Core not halted after single step")
    ...
    def step(self):
        ...
        self.write_memory(0xE000EDF0, 4, (0xA05F << 16) | (1 << 2) | (1 << 0))

        # Look at the core acknowledges by reading S_HALT bit in DHCSR
        DHCSR = self.read_memory(0xE000EDF0, 4)
        if not (DHCSR & 0x20000):
            self.log.warning("Core not halted after single step")
    ...
    def check_halt(self):
        ...
        # Check the S_HALT bit in the DHCSR register.
    ...
    def read_memory(self, address, wordsize=4, num_words=1, raw=False):
        ...
        while (nb_packet > 0):
            ...
            burst = nb_packet if nb_packet < 4 else burst_size
            nb_packet -= burst_size

            # Create a burst of 4 packets maximum
